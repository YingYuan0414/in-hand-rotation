...
class A2CBase(BaseAlgorithm):
	def __init__(self, base_name, params):
		...
		self.experiment_dir = os.path.join(self.experiment_dir, self.prefix)
	...
	def get_action_values(self, obs):
		...
		if isinstance(processed_obs, dict) and 'pointcloud' not in processed_obs:
			processed_obs = processed_obs['student_obs']
		elif isinstance(processed_obs, dict) and 'student_obs' in processed_obs:
			if 'pointcloud' in processed_obs:
				processed_obs = {'obs': processed_obs['student_obs'], 'pointcloud': processed_obs['pointcloud']}
	...
	def get_teacher_action_values(self, obs):
		processed_obs = self._preproc_obs(obs['obs'])
		if isinstance(processed_obs, dict):
			processed_obs = processed_obs['obs']
		...
		with torch.no_grad():
			res_dict = self.teacher(input_dict)
			if self.has_central_value:
				states = obs['states']
				input_dict = {
					'is_train': False,
	...
	def get_values(self, obs):
		with torch.no_grad():
			if self.has_central_value:
				...
				self.model.eval()
				if self.distill:
					if 'pointcloud' in obs['obs']:
						processed_obs = self._preproc_obs({'obs': obs['obs']['student_obs'],
					 									   'pointcloud': obs['obs']['pointcloud']})
					else:
						processed_obs = self._preproc_obs(obs['obs']['student_obs'])
				else:
					processed_obs = self._preproc_obs(obs['obs'])
				input_dict = {
					'is_train': False,
	...
	def init_tensors(self):
		...
		if self.is_rnn:
			self.rnn_states = self.model.get_default_rnn_state() # [LAYERS, SEQ_LENGTH, HIDDEN_DIM]
	...
	def set_full_state_weights(self, weights, mode='student'):
		self.set_weights(weights, mode)
		if mode == 'teacher':
			return
		if 'epoch' in weights:
	...
	def set_stats_weights(self, weights, mode='student'):
		if self.normalize_rms_advantage:
			self.advantage_mean_std.load_state_dict(weights['advantage_mean_std'])
		if self.normalize_input and 'running_mean_std' in weights:
			if mode == 'teacher':
				self.teacher.running_mean_std.load_state_dict(weights['running_mean_std'])
			else:
				self.model.running_mean_std.load_state_dict(weights['running_mean_std'])
		if self.normalize_value and 'normalize_value' in weights:
			if mode == 'teacher':
				self.teacher.value_mean_std.load_state_dict(weights['reward_mean_std'])
			else:
				self.model.value_mean_std.load_state_dict(weights['reward_mean_std'])
		if self.mixed_precision and 'scaler' in weights:
			self.scaler.load_state_dict(weights['scaler'])

	def set_weights(self, weights, mode='student'):
		if mode == 'teacher':
			self.teacher.load_state_dict(weights['model'])
		else:
			self.model.load_state_dict(weights['model'])
		self.set_stats_weights(weights, mode)
	...
	def play_steps_rnn(self):
		...
		for n in range(self.horizon_length):
			...
			if n % self.seq_len == 0:
				for s, mb_s in zip(self.rnn_states, mb_rnn_states):
					mb_s[n // self.seq_len,:,:,:] = s # [SEQ_ID, 1, NUM_ENVS, HIDDEN_DIM]
		...
		for mb_s in mb_rnn_states:
			t_size = mb_s.size()[0] * mb_s.size()[2] # [SEQ_ID, 1, NUM_ENVS, HIDDEN_DIM]
			h_size = mb_s.size()[3]
