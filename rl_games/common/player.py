...
class BasePlayer(object):
    def __init__(self, params):
        ...
        self.action_savepath = self.player_config.get('player_action_savepath', './action_seq.pkl')
        print(self.action_savepath)
    ...
    def get_env_internal_state(self, env):
        return env.get_internal_state()

    def get_env_internal_info(self, env, key):
        return env.get_internal_info(key)
    ...
    def get_batch_size(self, obses, batch_size):
        ...
        if type(self.obs_shape) is dict:
            if 'obs' in obses and 'states' in obses:
                obses = obses['obs']
