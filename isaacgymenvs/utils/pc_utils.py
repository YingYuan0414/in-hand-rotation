...
# @timeit
def process_robot_pc(pc: torch.Tensor, bound: List[float], num_sampled_points=1000, segmentation: torch.Tensor=None):
    ...
    batch_pc = ops.sample_farthest_points(batch_pc, torch.tensor(within_indices_len, device=pc.device, dtype=torch.int),
                                            K=512)[0]
    batch_pc = torch.cat(
        [batch_pc, torch.zeros(batch_pc.shape[0], batch_pc.shape[1], 3).cuda()], dim=-1)
    batch_pc[:, :, 4] = 1
